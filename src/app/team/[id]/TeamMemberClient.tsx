          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="relative w-16 h-16 rounded-xl overflow-hidden border flex-shrink-0"
              ...
              <SafeImage src={member.profileImage} alt={member.name} fill className="rounded-xl"
                placeholder={<div className="absolute inset-0 flex items-center justify-center text-[#a3a3a3]"><FiUser size={24} /></div>} />
            </div>
            {/* 이름/역할 */}
            <div>
              <span className="section-label mb-1">Team Member</span>
              <h1 className="text-2xl sm:text-3xl font-bold text-[#262626] mb-1">{member.name}</h1>
              <p className="text-sm text-[#2a72e5]">{member.role}</p>
            </div>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10 max-w-6xl">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                ...
                <p className="text-sm text-[#4c4c4c] leading-relaxed max-w-2xl">{member.bio}</p>
              </motion.div>
              ...
              {memberProjects.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <h2 className="text-xs font-semibold text-[#5d5d5d] uppercase tracking-wider mb-4">참여 프로젝트</h2>
            ...
            <div className="lg:col-span-1">
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                ...
                <h2 className="text-xs font-semibold text-[#5d5d5d] uppercase tracking-wider mb-4">연락처</h2>
                <div className="space-y-3">
                  <a href={`mailto:${member.email}`}
                    className="flex items-center gap-3 text-[#4c4c4c] hover:text-[#262626] transition-colors group text-sm">
                    <FiMail size={14} />
                  ...
                  {member.linkedin && (
                    <a href={member.linkedin} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-3 text-[#4c4c4c] hover:text-[#2a72e5] transition-colors group text-sm">
                      <FiLinkedin size={14} />

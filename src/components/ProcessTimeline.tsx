...
// 다크 밴드 위에 렌더 — 화이트 텍스트, white/10 헤어라인, 모노크롬 아이콘 노드.
      ...
      <div className="space-y-0">
        {sortedSteps.map((step, index) => {
          ...
          return (
            ...
            >
              {/* 아이콘 노드 */}
              <div className="relative z-10 flex-shrink-0 w-10 h-10 rounded-sm bg-[#0a0a0a] border border-white/15 flex items-center justify-center text-white">
                <Icon size={17} />
              </div>

              <div className="min-w-0 pt-1">
                <div className="flex items-baseline gap-2.5 mb-1.5">
                  <span className="text-[12px] text-white/35 font-mono-stat">
                    {String(step.order).padStart(2, '0')}
                  </span>
                  <h3 className="text-[15px] font-semibold text-white">{step.title}</h3>
                </div>
                <p className="text-sm text-white/55 leading-[1.7] max-w-md">
                  {step.description}
                </p>
              </div>
            </motion.div>

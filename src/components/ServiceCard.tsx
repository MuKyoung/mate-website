    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: 'rgba(42,114,229,0.08)', border: '1px solid rgba(42,114,229,0.14)' }}
        >
          <span className="text-xl leading-none">{service.icon}</span>
        </div>

        {/* 타이틀 */}
        <h3 className="text-base font-semibold text-[#262626] mb-2.5 group-hover:text-[#2a72e5] transition-colors duration-200">
          {service.title}
        </h3>

        {/* 설명 */}
        <p className="text-sm text-[#5d5d5d] mb-5 leading-relaxed">
          {service.description}
        </p>
        ...
        <ul className="space-y-2 mb-6">
          {service.features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-2.5 text-sm text-[#5d5d5d]">
              <span className="flex-shrink-0 mt-0.5 w-3.5 h-3.5 rounded-sm flex items-center justify-center"
                style={{ background: 'rgba(42,114,229,0.12)' }}>
                <FiCheck className="text-[#2a72e5]" size={9} />
              </span>
              {feature}
            </li>
          ))}
        ...
        {/* 링크 */}
        <div className="flex items-center gap-1.5 text-xs font-medium text-[#5d5d5d] group-hover:text-[#2a72e5] transition-colors duration-200">
          <span>자세히 보기</span>

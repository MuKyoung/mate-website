...
export default function TeamMemberCard({ member, index }: TeamMemberCardProps) {
  return (
    <motion.div
      ...
      className="group h-full flex flex-col rounded-sm border border-[#e4e4e4] bg-white hover:border-[#0a0a0a] transition-colors"
    >
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <h3 className="text-[17px] font-semibold text-[#0a0a0a] truncate">{member.name}</h3>
              <FiArrowUpRight size={16} className="flex-shrink-0 mt-0.5 text-[#a1a1aa] group-hover:text-[#0a0a0a] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
            </div>
            <p className="text-[13px] text-[#52525b] mt-0.5">{member.role}</p>
          </div>
        </div>
      ...
      <div className="flex gap-4 px-6 py-4 border-t border-[#e4e4e4]">
        ...
        {member.linkedin && (
          <a href={member.linkedin} target="_blank" rel="noopener noreferrer"
            className="text-[#a1a1aa] hover:text-[#0a0a0a] transition-colors" aria-label="LinkedIn">
            <FiLinkedin size={16} />
          </a>
        )}
        <a href={`mailto:${member.email}`}
          className="text-[#a1a1aa] hover:text-[#0a0a0a] transition-colors" aria-label="Email">
          <FiMail size={16} />
        </a>
      </div>
    </motion.div>
